    ...
    methods: {
      ...
      getGpuLimit(item) {
        ...
        item.spec.template.spec.containers.forEach((c) => {
          ...
          if (Object.keys(c?.resources?.limits || {}).some((key) => key.indexOf('huawei.com') > -1)) {
            gpu.AscendNpu = 0;
            Object.keys(c?.resources?.limits || {})
              .filter((k) => k.indexOf('huawei.com') > -1)
              .forEach((k) => {
                gpu.AscendNpu += parseFloat(c?.resources?.limits && c?.resources?.limits[k]);
              });
          }
          if (
            (c?.resources?.limits && c?.resources?.limits['volcano.sh/vgpu-memory']) ||
            (c?.resources?.limits && c?.resources?.limits['volcano.sh/vgpu-number'])
          ) {
            const match = new RegExp('.*-S(\\d+)\\..*').exec(
              item.metadata.annotations?.[`pai.kubegems.io/sku-raw-name`] || '',
            );
            gpu.NvidiaGpu += parseFloat(match?.[1] || '0') / 100;
          }
