        ...
        <v-list class="pa-0 kubegems__tip" dense>
          <v-list-item>
            <v-list-item-content>
              ...
              <template v-if="item.AscendNpu">
                <v-list-item-title>
                  <div class="float-left">
                    <BaseLogo icon-name="huawei" :ml="0" />
                  </div>
                  <div class="logo"> Huawei Ascend </div>
                </v-list-item-title>
                <v-row>
                  <v-col class="py-1" cols="6">
                    <v-list-item class="float-left pa-0 gpu__item" two-line>
                      <v-list-item-content class="text-body-2 text--secondary">
                        {{ i18nLocal.t('tip.gpu_core') }} : {{ item.AscendNpu || 0 }} NPU
                        <div v-if="allocated">
                          {{ i18nLocal.t('tip.used') }} : {{ parseInt(item.AllocatedAscendNpu || 0) }} NPU
                          <v-progress-linear
                            class="rounded font-weight-medium mt-1"
                            :color="`${getColor(item.AscendNpuPercentage)} lighten-1`"
                            height="10"
                            :value="item.AscendNpuPercentage"
                          >
                            <span class="white--text">{{ item.AscendNpuPercentage }}% </span>
                          </v-progress-linear>
                        </div>
                      </v-list-item-content>
                    </v-list-item>
