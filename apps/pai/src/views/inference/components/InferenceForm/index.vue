...
<template>
  <BaseDialog
    ...
    :title="
      state.edit
        ? i18n.t('operate.update_c', [i18nLocal.t('resource.task')])
        : i18n.t('operate.add_c', [i18nLocal.t('resource.task')])
    "
    :width="1200"
    @reset="reset"
  >
...
<script lang="ts" setup>
  ...
  const _unique = (arr) => {
    ...
    arr.forEach((item) => {
      if (
        newArr.findIndex((a) => {
          return (
            a.kind === item.kind && a.name === item.name && a.targetPath === item.targetPath && a.source === item.source
          );
        }) === -1
      ) {
        newArr.push(item);
  ...
  const confirm = async (): Promise<void> => {
    if (form.value.validate()) {
      ...
      obj.value.config.queue = store.getters.Tenant().TenantName;
      obj.value.config.roles[0].name = 'service';
      obj.value.config.kind = 'inference';
      if (state.edit) {
        obj.value.config.roles.forEach((r, index) => {
          r.volumeMountSets = r.volumeMountSets.concat(_unique(state.otherVolumeMount[index] || []));
        });
