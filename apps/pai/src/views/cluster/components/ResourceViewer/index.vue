  ...
  <div>
    <v-row>
      ...
      <v-col cols="3">
        <v-row>
          <v-col cols="12">
            <FileCard :data-map="fileTenantMap" :title="i18nLocal.t('tip.files').toString()" type="file" />
          </v-col>
          <v-col class="pt-0" cols="12">
            <FileCard
              :data-map="capabilityTenantMap"
...
<script lang="ts" setup>
  ...
  import {
    PAI_SET_CAPABILITY_BY_CLUSTER_PROMQL,
    PAI_SET_CAPABILITY_TENANT_PROMQL,
    PAI_SET_COUNT_BY_CLUSTER_PROMQL,
    PAI_SET_FILES_BY_CLUSTER_PROMQL,
    PAI_SET_FILES_TENANT_PROMQL,
  ...
  const getResource = async (): Promise<void> => {
    const resource = await new Region({
  ...
  const getSetCount = async (): Promise<void> => {
    const query = PAI_SET_COUNT_BY_CLUSTER_PROMQL.replaceAll('%', '');
  ...
  const getFile = async (): Promise<void> => {
    const query = PAI_SET_FILES_BY_CLUSTER_PROMQL.replaceAll('%', '');
    const data = await new Vector().getVector(props.region.clusterName, { query: query, noprocessing: true });
    data.forEach((d) => {
  ...
  const fileTenantMap = ref<any>(void 0);
  const getFileByTenant = async (): Promise<void> => {
    const query = PAI_SET_FILES_TENANT_PROMQL.replaceAll('%', '');
    const data = await new Vector().getVector(props.region.clusterName, { query: query, noprocessing: true });
    const _file = {};
    data.forEach((d) => {
      if (d.metric?.tenant) _file[d.metric?.tenant] = d.value[1];
    });
    fileTenantMap.value = _file;
  };
  ...
  const getCapability = async (): Promise<void> => {
    const query = PAI_SET_CAPABILITY_BY_CLUSTER_PROMQL.replaceAll('%', '');
    const data = await new Vector().getVector(props.region.clusterName, { query: query, noprocessing: true });
    data.forEach((d) => {
  ...
  const capabilityTenantMap = ref<any>(void 0);
  const getCapabilityByTenant = async (): Promise<void> => {
    const query = PAI_SET_CAPABILITY_TENANT_PROMQL.replaceAll('%', '');
    const data = await new Vector().getVector(props.region.clusterName, { query: query, noprocessing: true });
    const _capability = {};
    data.forEach((d) => {
      if (d.metric?.tenant) _capability[d.metric?.tenant] = d.value[1];
    });
    capabilityTenantMap.value = _capability;
  };
