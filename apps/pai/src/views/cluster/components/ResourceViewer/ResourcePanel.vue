    ...
    <template #content>
      ...
      >
        <div class="ma-4 text-body-1">
          <div>{{ i18nLocal.t('tip.max_resource') }}: {{ used }} / {{ total }}</div>
        </div>
...
<script lang="ts" setup>
  ...
  import { JobResource } from '../../../../api/tenant';
  ...
  const title = computed(() => {
    return `${props.region?.clusterName || ''} ${titleMap[state.type]} ${i18nLocal.t('tip.used_detail')}`;
  });
  ...
  const getJobList = async (): Promise<void> => {
    const data = await new Region({
      name: props.region.name,
    }).getRegionSummaryByResource(state.type, { page: 1, size: 1000 });
    jobItems.value = [];
    tabCount.value = {
      modeling: 0,
      training: 0,
    ...
    jobItems.value.forEach((item) => {
      const _m1 = m1.find((m) => {
        return m.metric.pod.startsWith(`${item?.labels?.['pai.kubegems.io/tenant'] || ''}-${item.name}`);
      });
      const _m2 = m2.find((m) => {
        return m.metric.pod.startsWith(`${item?.labels?.['pai.kubegems.io/tenant'] || ''}-${item.name}`);
      });
  ...
  const podCPUMAXUsage = async (): Promise<Vector[]> => {
    const query = POD_MAX_CPU_USAGE_DURATION_PROMQL.replaceAll(
      '$1',
      jobItems.value
        .map((j) => {
          return `.*-${j.name}.*`;
  ...
  const podCPUAVGUsage = async (): Promise<Vector[]> => {
    const query = POD_AVG_CPU_USAGE_DURATION_PROMQL.replaceAll(
      '$1',
      jobItems.value
        .map((j) => {
          return `.*-${j.name}.*`;
  ...
  const podMemoryMAXUsage = async (): Promise<Vector[]> => {
    const query = POD_MAX_MEMORY_USAGE_DURATION_PROMQL.replaceAll(
      '$1',
      jobItems.value
        .map((j) => {
          return `.*-${j.name}.*`;
  ...
  const podMemoryAVGUsage = async (): Promise<Vector[]> => {
    const query = POD_AVG_MEMORY_USAGE_DURATION_PROMQL.replaceAll(
      '$1',
      jobItems.value
        .map((j) => {
          return `.*-${j.name}.*`;
  ...
  const podGPUMAXUsage = async (): Promise<Vector[]> => {
    const query = NVIDIA_CPU_MAX_DURATION_PROMQL.replaceAll(
      '$1',
      jobItems.value
        .map((j) => {
          return `.*-${j.name}.*`;
  ...
  const podGPUMemoryMAXUsage = async (): Promise<Vector[]> => {
    const query = NVIDIA_MEMORY_MAX_DURATION_PROMQL.replaceAll(
      '$1',
      jobItems.value
        .map((j) => {
          return `.*-${j.name}.*`;
