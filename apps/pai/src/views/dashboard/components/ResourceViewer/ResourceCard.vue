...
<template>
  <v-card class="white" elevation="0" height="200px">
    <div class="d-flex align-center px-4 pt-3">
      ...
      <div class="mx-4">
        <h4 class="text-h6 primary--text"> {{ title }} </h4>
      </div>
    ...
    <div class="px-5 py-2">
      ...
      <BaseAreaChart
        v-if="chartValues && chartValues.length > 0"
...
<script lang="ts" setup>
  ...
  import {
    PAI_JOB_COUNT_GPU_PROMQL,
    PAI_JOB_COUNT_PROMQL,
    PAI_JOB_USAGE_CPU_PROMQL,
    PAI_JOB_USAGE_MEMORY_PROMQL,
  ...
  const getMetrics = async () => {
    let query = '';
    if (props.type === 'gpu') {
      query = PAI_JOB_COUNT_GPU_PROMQL.replaceAll('$1', store.getters.Tenant().TenantName);
    } else if (props.type === 'job') {
      query = PAI_JOB_COUNT_PROMQL.replaceAll('$1', store.getters.Tenant().TenantName);
    } else if (props.type === 'cpu') {
      query = PAI_JOB_USAGE_CPU_PROMQL.replaceAll('$1', store.getters.Tenant().TenantName);
    } else if (props.type === 'memory') {
      query = PAI_JOB_USAGE_MEMORY_PROMQL.replaceAll('$1', store.getters.Tenant().TenantName);
    } else {
      return;
    }
