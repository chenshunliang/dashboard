          ...
          <v-col cols="3">
            <div :class="`clear-zoom-${store.state.Scale.toString().replaceAll('.', '-')}`">
              <BasePieChart
                ...
                :title="`用户${titleMap[state.type]}文件总数`"
                type="doughnut"
                unit-type="number"
              />
            </div>
          ...
          <v-col cols="3">
            <div :class="`clear-zoom-${store.state.Scale.toString().replaceAll('.', '-')}`">
              <BasePieChart
                ...
                :title="`用户${titleMap[state.type]}数量`"
                type="doughnut"
                unit-type="number"
              />
            </div>
          ...
          <v-col cols="3">
            <div :class="`clear-zoom-${store.state.Scale.toString().replaceAll('.', '-')}`">
              <BasePieChart
                ...
                :title="`用户${titleMap[state.type]}文件容量`"
                type="doughnut"
                unit-type="storage"
              />
            </div>
          ...
          <v-col cols="3">
            <div :class="`clear-zoom-${store.state.Scale.toString().replaceAll('.', '-')}`">
              <BasePieChart
                ...
                :title="`${titleMap[state.type]}文件总数Top10`"
                type="doughnut"
                unit-type="number"
              />
            </div>
          ...
          <v-col cols="3">
            <div :class="`clear-zoom-${store.state.Scale.toString().replaceAll('.', '-')}`">
              <BasePieChart
                ...
                :title="`${titleMap[state.type]}文件容量Top10`"
                type="doughnut"
                unit-type="storage"
              />
            </div>
...
<script lang="ts" setup>
  ...
  import {
    PAI_SET_CAPABILITY_BY_USER_PROMQL,
    PAI_SET_COUNT_BY_USER_PROMQL,
    PAI_SET_FILES_BY_USER_PROMQL,
    PAI_SET_TOP_CAPABILITY_PROMQL,
    PAI_SET_TOP_FILES_PROMQL,
  ...
  const title = computed(() => {
    return `${store.getters.Tenant().TenantName} ${titleMap[state.type]}`;
  });
  ...
  const getFilesByUserList = async (): Promise<void> => {
    let query = PAI_SET_FILES_BY_USER_PROMQL.replaceAll('$1', store.getters.Tenant().TenantName).replaceAll(
      '$2',
  ...
  const getFileCountByUserList = async (): Promise<void> => {
    let query = PAI_SET_COUNT_BY_USER_PROMQL.replaceAll('$1', store.getters.Tenant().TenantName).replaceAll(
      '$2',
      state.type === 'all' ? 'workspace|modelset|dataset' : state.type,
    );
    query = query.replaceAll('%', '');
  ...
  const getCapabilityByUserList = async (): Promise<void> => {
    let query = PAI_SET_CAPABILITY_BY_USER_PROMQL.replaceAll('$1', store.getters.Tenant().TenantName).replaceAll(
      '$2',
      state.type === 'all' ? 'workspace|modelset|dataset' : state.type,
    );
    query = query.replaceAll('%', '');
  ...
  const getFileTopByUserList = async (): Promise<void> => {
    let query = PAI_SET_TOP_FILES_PROMQL.replaceAll('$1', store.getters.Tenant().TenantName).replaceAll(
      '$2',
      state.type === 'all' ? 'workspace|modelset|dataset' : state.type,
    );
    query = query.replaceAll('%', '');
  ...
  const getCapabilityTopByUserList = async (): Promise<void> => {
    let query = PAI_SET_TOP_CAPABILITY_PROMQL.replaceAll('$1', store.getters.Tenant().TenantName).replaceAll(
      '$2',
      state.type === 'all' ? 'workspace|modelset|dataset' : state.type,
    );
    query = query.replaceAll('%', '');
