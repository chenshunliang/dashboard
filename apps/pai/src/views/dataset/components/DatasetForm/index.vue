...
<template>
  <BaseDialog
    ...
    :title="
      state.edit
        ? i18n.t('operate.update_c', [i18nLocal.t('resource.dataset')])
        : i18n.t('operate.create_c', [i18nLocal.t('resource.dataset')])
    "
    :width="800"
    @reset="reset"
  >
...
<script lang="ts" setup>
  ...
  const obj = ref<Dataset>(new Dataset());

  const refresh = (item: Dataset) => {
  ...
  const init = async (item: Dataset): Promise<void> => {
    state.edit = true;
    const data = await new Dataset(item).getDataset();
    if (!data.config) {
  ...
  const confirm = async (): Promise<void> => {
    if (form.value.validate(true)) {
      ...
      if (state.edit) {
        await new Dataset(obj.value).updateDataset();
      } else {
        await new Dataset(obj.value).addDataset();
      }
      reset();
      emit('refresh');
    }
