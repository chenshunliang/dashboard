  ...
  <v-container fluid>
            ...
            <v-card>
              <v-card-text class="pa-2">
                <v-flex v-if="job.status ? job.config.paused : ''">
                  <v-btn color="primary" small text @click="updateDevelop">
                    {{ i18n.t('operate.edit') }}
                  </v-btn>
                </v-flex>
                <v-flex v-if="job.status ? job.config.paused : ''">
                  <v-btn color="primary" small text @click="startDevelop">
                    {{ i18nLocal.t('operate.start') }}
                  </v-btn>
                </v-flex>
                <v-flex v-else>
                  <v-btn color="warning" small text @click="stopDevelop">
                    {{ i18nLocal.t('operate.stop') }}
                  </v-btn>
                </v-flex>
                <v-flex>
                  <v-btn color="error" small text @click="removeDevelop">
    ...
    <v-row class="mt-0">
      ...
      <v-col class="pt-0" cols="10">
        ...
        <component :is="tabItems[tab].value" class="mt-3" :job="job" />
      </v-col>
    </v-row>

    <DevelopForm ref="develop" @refresh="getJobDetail" />
  </v-container>
</template>
...
<script lang="ts" setup>
  ...
  const tabItems = [
    { text: i18nLocal.t('tab.basic_info'), value: ResourceInfo, tab: 'info' },
  ...
  const develop = ref(null);
  const updateDevelop = (): void => {
    develop.value.init(job.value);
    develop.value.open();
  };

  const stopDevelop = (): void => {
    store.commit('SET_CONFIRM', {
      title: i18nLocal.t('operate.stop_c', [i18nLocal.t('resource.instance')]),
      content: {
        text: `${i18nLocal.t('operate.stop_c', [i18nLocal.t('resource.instance')])} ${job.value.name}`,
        type: 'confirm',
      },
      param: {},
      doFunc: async () => {
  ...
  const startDevelop = (): void => {
    store.commit('SET_CONFIRM', {
      title: i18nLocal.t('operate.start_c', [i18nLocal.t('resource.instance')]),
      content: {
        text: `${i18nLocal.t('operate.start_c', [i18nLocal.t('resource.instance')])} ${job.value.name}`,
        type: 'confirm',
      },
      param: {},
      doFunc: async () => {
  ...
  const removeDevelop = (): void => {
    store.commit('SET_CONFIRM', {
      title: i18n.t('operate.delete_c', [i18nLocal.t('resource.instance')]),
      content: {
        text: `${i18n.t('operate.delete_c', [i18nLocal.t('resource.instance')])} ${job.value.name}`,
        type: 'delete',
        name: job.value.name,
      },
      param: {},
