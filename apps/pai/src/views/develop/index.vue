...
<template>
  <v-container fluid>
    ...
    <v-card v-if="useQueueCheck()">
        ...
        <v-menu left>
          ...
          <v-card>
            <v-card-text class="pa-2">
              <v-flex>
                <v-btn color="primary" text @click="addDevelopJob">
                  <v-icon left>mdi-plus-box</v-icon>
                  {{ i18n.t('operate.create_c', [i18nLocal.t('resource.im')]) }}
                </v-btn>
              </v-flex>
              <v-flex>
                <v-btn color="primary" text @click="bacthStopOrStartJobs(true)">
                  <v-icon left>mdi-play</v-icon>
                  {{ i18n.t('operate.batch_start_c', [i18nLocal.t('resource.instance')]) }}
                </v-btn>
              </v-flex>
              <v-flex>
                <v-btn color="warning" text @click="bacthStopOrStartJobs(false)">
                  <v-icon left>mdi-pause</v-icon>
                  {{ i18n.t('operate.batch_stop_c', [i18nLocal.t('resource.instance')]) }}
                </v-btn>
              </v-flex>
              <v-flex>
                <v-btn color="error" text @click="batchDeleteJobs">
                  <v-icon left>mdi-minus-box</v-icon>
                  {{ i18n.t('operate.batch_delete_c', [i18nLocal.t('resource.instance')]) }}
                </v-btn>
              </v-flex>
            </v-card-text>
          </v-card>
      ...
      >
        <template #item.name="{ item }">
          <div>
            <a class="text-subtitle-2" @click.stop="toDevelopJobDetail(item)">
          ...
          <div class="text-caption kubegems__text">
            ...
            >
              <template #activator="{ on }">
                <div class="float-left table__desc" v-on="on">
            ...
            <div v-else class="float-left table__desc"> {{ item.description || '-' }} </div>
        ...
        <template #item.address="{ item }">
          <v-chip v-for="(port, index) in getAddresses(item)" :key="index" color="primary" small>
            {{ port && port.access && port.access.host ? port.access.host : '' }}
            <v-btn color="white" icon small @click="toAddress(port)">
        ...
        <template #item.action="{ item, index }">
          ...
          <v-menu :attach="`#r-task${index}`" left>
            ...
            <v-card>
              <v-card-text class="pa-2">
                <v-flex>
                  <v-btn color="primary" small text @click="viewDevelopJob(item)">
                    {{ i18n.t('operate.view') }}
                  </v-btn>
                </v-flex>
                <v-flex>
                  <v-btn color="primary" small text @click="updateDevelopJob(item)">
                    {{ i18n.t('operate.edit') }}
                  </v-btn>
                </v-flex>
                <v-flex v-if="item.config.paused">
                  <v-btn color="primary" small text @click="startDevelopJob(item)">
                    {{ i18nLocal.t('operate.start') }}
                  </v-btn>
                </v-flex>
                <v-flex v-else>
                  <v-btn color="warning" small text @click="stopDevelopJob(item)">
                    {{ i18nLocal.t('operate.stop') }}
                  </v-btn>
                </v-flex>
                <v-flex>
                  <v-btn color="error" small text @click="removeDevelopJob(item)">
    ...
    <DevelopForm ref="task" @refresh="getDevelopJobList" />
...
<script lang="ts" setup>
  ...
  import DevelopForm from './components/DevelopForm/index.vue';
  ...
  const filterCreatorChanged = (): void => {
    getDevelopJobList({ page: 1, size: 10, noprocessing: false });
  };

  const getDevelopJobList = async (params: KubePaginationRequest = pagination): Promise<void> => {
  ...
  watch(
    ...
    async (newValue) => {
      ...
      if (_cancel) _cancel();
      const { cancel } = useRequest(getDevelopJobList, {
  ...
  const toDevelopJobDetail = (item: Job): void => {
    router.push({
  ...
  const task = ref(null);
  const addDevelopJob = (): void => {
    task.value.open();
  };

  const updateDevelopJob = (item: Job): void => {
    task.value.init(item);
    task.value.open();
  };

  const stopDevelopJob = (item: Job): void => {
  ...
  const startDevelopJob = (item: Job): void => {
    store.commit('SET_CONFIRM', {
      title: i18nLocal.t('operate.start_c', [i18nLocal.t('resource.instance')]),
      content: {
        text: `${i18nLocal.t('operate.start_c', [i18nLocal.t('resource.instance')])} ${item.name}`,
        type: 'confirm',
      },
      param: { item },
      doFunc: async (param) => {
  ...
  const removeDevelopJob = (item: Job): void => {
    store.commit('SET_CONFIRM', {
      title: i18n.t('operate.delete_c', [i18nLocal.t('resource.instance')]),
      content: {
        text: `${i18n.t('operate.delete_c', [i18nLocal.t('resource.instance')])} ${item.name}`,
        type: 'delete',
        name: item.name,
      },
      param: { item },
  ...
  const bacthStopOrStartJobs = (toStart = true): void => {
    ...
    if (filteredJobList.length === 0) {
      if (toStart) {
        store.commit('SET_SNACKBAR', {
          text: i18nLocal.t('tip.batch_select_stop_c', [i18nLocal.t('resource.im')]),
          color: 'warning',
        });
        return;
      } else {
        store.commit('SET_SNACKBAR', {
          text: i18nLocal.t('tip.batch_select_start_c', [i18nLocal.t('resource.im')]),
          color: 'warning',
        });
        return;
      }
  ...
  const getIcon = (item: Job): string => {
    if (item.annotations?.['pai.kubegems.io/im-type'] === 'jupyter') return 'jupyter';
    if (item.annotations?.['pai.kubegems.io/im-type'] === 'code-server') return 'vscode';
    const image = item.config.roles[0]?.image;
    if (image?.indexOf('jupyter') > -1) return 'jupyter';
    if (image?.indexOf('code-server') > -1) return 'vscode';
    return 'mdi:graphql';
  };
  ...
  const viewDevelopJob = (item: Job): void => {
    yaml.value = item.raw;
