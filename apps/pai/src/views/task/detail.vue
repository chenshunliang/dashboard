...
<template>
  <v-container fluid>
            ...
            <v-card>
              <v-card-text class="pa-2">
                <v-flex v-if="job.status ? job.config.paused : ''">
                  <v-btn color="primary" small text @click="updateTask">
                    {{ i18n.t('operate.edit') }}
                  </v-btn>
                </v-flex>
                <v-flex v-if="job.status ? job.config.paused : ''">
                  <v-btn color="primary" small text @click="startTask">
                    {{ i18nLocal.t('operate.start') }}
                  </v-btn>
                </v-flex>
                <v-flex v-else>
                  <v-btn color="warning" small text @click="stopTask">
                    {{ i18nLocal.t('operate.stop') }}
                  </v-btn>
                </v-flex>
                <v-flex>
                  <v-btn color="error" small text @click="removeTask">
    ...
    <TaskFrom ref="task" @refresh="getJobDetail" />
  </v-container>
</template>
...
<script lang="ts" setup>
  ...
  import ResourceInfo from './components/ResourceInfo/index.vue';
  import TaskFrom from './components/TaskForm/index.vue';
  import TaskMonitor from './components/TaskMonitor.vue';
  import VisualList from './components/VisualList/index.vue';
  ...
  const tabItems = [
    ...
    { text: i18nLocal.t('tab.visual'), value: VisualList, tab: 'visual' },
