            ...
            >
              <v-list-item-content class="py-0">
                ...
                <v-list-item-content class="text-caption kubegems__text kubegems__break-all">
                  {{ i18nLocal.t('tip.duration') }}:{{
                    formatDuring(
                      Date.parse(new Date(item.finishTimestamp || 0).toString()) -
                        Date.parse(new Date(item.startTimestamp || 0).toString()),
                    )
                  }}
                </v-list-item-content>
...
<script lang="ts" setup>
  ...
  import { formatDuring } from '@kubegems/libs/utils/helpers';
