...
<template>
  <v-container fluid>
    ...
    <v-card v-if="useQueueCheck()">
        ...
        <v-menu left>
          ...
          <v-card>
            <v-card-text class="pa-2">
              <v-flex>
                <v-btn color="primary" text @click="addTask">
      ...
      >
        <template #item.name="{ item }">
          <div>
            <a class="text-subtitle-2" @click.stop="toTaskDetail(item)">
              {{ item.displayName || item.name }}
            </a>
            <v-btn v-if="item.visualConfig" color="primary" icon small @click="toBoard(item.visualConfig)">
              <BaseLogo
                :color="store.state.ThemeColor"
                default-logo="none"
                :icon-name="isTensor(item) ? 'tensorflow' : 'ray'"
                :ml="0"
                :mt="1"
                :width="18"
              />
          ...
          <v-menu
            v-if="item.description.length > 45"
            :close-delay="200"
            max-width="300"
            min-width="80"
            nudge-right="20px"
        ...
        <template #item.lastStartupAt="{ item, index }">
          <History :job="item" :top="pagination.size - index <= 5 || (pagination.items.length <= 5 && index >= 1)">
            <template #trigger>
              {{ getLastTriggerTime(item) || moment(item.createdAt, 'YYYY-MM-DDTHH:mm:ssZ').format('lll') }}
            </template>
          </History>
        </template>
        <template #item.duration="{ item, index }">
          <Duration :job="item" :top="pagination.size - index <= 5 || (pagination.items.length <= 5 && index >= 1)">
            <template #trigger>
              {{ getLastDurationTime(item) || '0' }}
            </template>
          </Duration>
        </template>
        <template #item.action="{ item, index }">
          ...
          <v-menu :attach="`#r-task${index}`" left>
            ...
            <v-card>
              <v-card-text class="pa-2">
                <v-flex>
                  <v-btn color="primary" small text @click="viewTask(item)">
                    {{ i18n.t('operate.view') }}
                  </v-btn>
                </v-flex>
                <v-flex>
                  <v-btn color="primary" small text @click="updateTask(item)">
                    {{ i18n.t('operate.edit') }}
                  </v-btn>
                </v-flex>
                <v-flex v-if="item.config.paused">
                  <v-btn color="primary" small text @click="startTask(item)">
                    {{ i18nLocal.t('operate.start') }}
                  </v-btn>
                </v-flex>
                <v-flex v-else>
                  <v-btn color="warning" small text @click="stopTask(item)">
                    {{ i18nLocal.t('operate.stop') }}
                  </v-btn>
                </v-flex>
                <v-flex v-if="item.status.state.phase === 'Pending'">
                  <v-btn color="primary" small text @click="changePriority(item)">
                    {{ i18nLocal.t('operate.change_priority') }}
                  </v-btn>
                </v-flex>
                <v-flex>
                  <v-btn color="error" small text @click="removeTask(item)">
    ...
    <TaskFrom ref="task" @refresh="getTaskList" />
    <ChangePriority ref="priority" @refresh="getTaskList" />
...
<script lang="ts" setup>
  ...
  import { convertStrToNum, deepCopy, formatDuring, sleep } from '@kubegems/libs/utils/helpers';
  ...
  import ChangePriority from './components/ChangePriority.vue';
  import Duration from './components/Duration.vue';
  import History from './components/History.vue';
  import TaskFrom from './components/TaskForm/index.vue';
  import TaskStatusTip from './components/TaskStatusTip.vue';
  ...
  const headers = [
    { text: i18nLocal.t('table.name'), value: 'name', align: 'start', width: '350px' },
    ...
    { text: i18nLocal.t('table.last_startup_at'), value: 'lastStartupAt', align: 'start' },
  ...
  const filterCreatorChanged = (): void => {
    getTaskList({ page: 1, size: 10, noprocessing: false });
  };

  const getTaskList = async (params: KubePaginationRequest = pagination): Promise<void> => {
    ...
    const visualConfigList = await new VisualConfig({
      region: store.getters.Region().RegionName,
      tenant: store.getters.Tenant().TenantName,
    }).getVisualConfigList({ page: 1, size: 1000, noprocessing: true });
    convertResponse2List(visualConfigList).forEach((vc) => {
      pagination.items.findIndex((item) => {
        if (item.name === vc.name) {
          item.visualConfig = vc;
        }
      });
    });
  ...
  watch(
    ...
    async (newValue) => {
      ...
      if (_cancel) _cancel();
      const { cancel } = useRequest(getTaskList, {
  ...
  const toTaskDetail = (item: Job): void => {
    router.push({
  ...
  const getTaskType = (roles: Role[]): string => {
    if (roles.length > 1) {
      return i18nLocal.t('tip.multi_n_multi_g').toString();
    }
    const single = roles.some((role) => {
  ...
  const getLastTriggerTime = (item: Job): string => {
    let [history] = item.status?.history || [];
    if (item.status.state.phase === 'Completed') {
      history = item.status.state;
    }
    if (history) {
      return history.startTimestamp ? moment(history.startTimestamp, 'YYYY-MM-DDTHH:mm:ssZ').format('lll') : void 0;
    }
    return void 0;
  };

  const getLastDurationTime = (item: Job): string => {
    if (item.status.state.phase === 'Running') {
      const s = new Date(item.status.state.runningTimestamp || item.status.state.startTimestamp);
      const e = new Date();
  ...
  const updateTask = (item: Job): void => {
    task.value.init(item);
    task.value.open();
  };

  const stopTask = (item: Job): void => {
  ...
  const removeTask = (item: Job): void => {
    store.commit('SET_CONFIRM', {
  ...
  const changePriority = (item: Job): void => {
    priority.value.init(item);
    priority.value.open();
  };
  ...
  const bacthStopOrStartJobs = (toStart = true): void => {
    ...
    if (filteredJobList.length === 0) {
      if (toStart) {
        store.commit('SET_SNACKBAR', {
          text: i18nLocal.t('tip.batch_select_stop_c', [i18nLocal.t('tip.job')]),
          color: 'warning',
        });
        return;
      } else {
        store.commit('SET_SNACKBAR', {
          text: i18nLocal.t('tip.batch_select_start_c', [i18nLocal.t('tip.job')]),
          color: 'warning',
        });
        return;
      }
  ...
  const toBoard = (item: VisualConfig) => {
    const tensor = item?.status?.tensorboard?.host;
    const [ray] = item?.status?.portExposure?.portStates || [];
    const url = tensor || ray?.url || item?.status?.host;
    if (url) window.open(url);
    else
      store.commit('SET_SNACKBAR', {
        text: i18nLocal.t('tip.no_address'),
        color: 'warning',
      });
  };
  ...
  const isTensor = (item: VisualConfig): boolean => {
