  ...
  <v-container fluid>
    ...
    <template v-if="useQueueCheck()">
      <v-card>
              ...
              <v-card-text class="pa-2">
                <v-flex>
                  <v-btn color="primary" text @click="addImage">
                    ...
                    {{ i18n.t('operate.create_c', [i18nLocal.t('resource.self')]) }}
                  </v-btn>
                </v-flex>
              </v-card-text>
            </v-card>
        ...
        <v-card-text>
          <v-tabs v-model="tab" class="rounded-t" height="30" @change="tabChanged">
...
<script lang="ts" setup>
  ...
  import { ComputedRef, computed, onMounted, onUnmounted, reactive, ref } from 'vue';
  ...
  const tabItems = [
    { text: i18nLocal.t('resource.self'), value: 'self' },
    { text: i18nLocal.t('resource.opensource'), value: 'opensource' },
  ];
  ...
  const getImageList = async (): Promise<void> => {
    let data = [];
    if (tab.value == 0) {
      data = await new Image({ tenant: store.getters.Tenant().TenantName }).getImageList({
        noprocessing: pagination.noprocessing,
      });
    } else {
      data = await new Image().getPublicImageList({ noprocessing: pagination.noprocessing });
    }
  ...
  const addImage = (): void => {
    image.value.open(tab.value !== 0);
  };

  const updateImage = (item: Image): void => {
    image.value.init(item, tab.value !== 0);
    image.value.open(tab.value !== 0);
  };

  const removeImage = (item: Image): void => {
    store.commit('SET_CONFIRM', {
      title: i18n.t('operate.delete_c', [i18nLocal.t('resource.self')]),
      content: {
        text: `${i18n.t('operate.delete_c', [i18nLocal.t('resource.self')])} ${item.name}`,
        type: 'delete',
        name: item.name,
      },
      param: { item },
      doFunc: async (param) => {
        if (param.item.name.length > 0) {
          await new Image(param.item).deleteImage();
          await sleep(500);
          getImageList();
        }
      },
